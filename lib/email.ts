...
const SENDER_NAME = process.env.BREVO_SENDER_NAME || "NACOSS FUTA Chapter Elections";
...
export async function sendRegistrationReceivedEmail(to: string, name: string) {
  const htmlContent = `
    <p>Hello ${name},</p>
    <p>We have received your voter registration for the NACOSS FUTA Chapter Elections.</p>
    ...
    <p>Thank you,</p>
    <p>NACOSS FUTA Chapter Electoral Committee</p>
  `;
  await sendEmail(to, "NACOSS FUTA Chapter Voter Registration Received", htmlContent);
}
...
export async function sendVerificationResultEmail(
  ...
  const htmlContent =
    status === "verified"
      ? `
        <p>Congratulations,</p>
        <p>Your voter registration has been approved! You are eligible to vote in the upcoming NACOSS FUTA Chapter Elections.</p>
